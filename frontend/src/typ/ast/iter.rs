    ...
    let range = match &for_loop.range {
        ast::ForLoopRange::UpTo(range) => {
            let (init, counter_ty) = match &range.init {
                ast::ForLoopCounterInit::Binding { binding_kw_span, name, init, ty, assign_op_span } => {
                    let (counter_ty, init_expr) = if ty.is_known() {
                        let counter_ty = typecheck_typename(ty, ctx)?;
                        let mut init_expr = typecheck_expr(init, counter_ty.ty(), ctx)?;
                        init_expr = implicit_conversion(init_expr, counter_ty.ty(), ctx)?;

                        (counter_ty, init_expr)
                    } else {
                        let init_expr = typecheck_expr(init, &DEFAULT_COUNTER_TY, ctx)?;
                        let counter_ty = TypeName::Inferred(init_expr.annotation().ty().into_owned());
                ...
                ast::ForLoopCounterInit::Assignment { counter, value, assign_op_span } => {
            ...
            let to_expr = typecheck_expr(&range.to_expr, counter_ty.ty(), ctx)?;
        ...
        ast::ForLoopRange::InSequence(range) => {
            ...
            let expect_src_ty = match &range.src_expr {
                ...
                _ => {
                    binding_ty.ty().clone().dyn_array()
                }
            };

            let mut seq_expr = typecheck_expr(&range.src_expr, &expect_src_ty, ctx)?;
            ...
            match (seq_expr.annotation().ty().as_ref(), binding_ty.is_inferred()) {
