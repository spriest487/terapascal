...
impl Vm {
    ...
    fn metadata(&self) -> &ir::Metadata {
        self.heap.marshaller.metadata()
    }
            ...
            ir::Ref::Global(name) => {
                ...
                match global {
                    GlobalValue::Variable { value, .. } => {
                        let byte_count = self.heap.marshaller.marshal(&val, value.as_mut())?;
    ...
    fn load_dyn_array_ptr(&self, ptr: &Pointer) -> ExecResult<(ArrayValue, ObjectHeader)> {
        let object_value = self.heap.load_object(ptr)?;

        let elements_array = match object_value.value {
            DynValue::Array(array_val) => *array_val,

            _ => {
                let type_name = object_value.header.id.to_pretty_name(&self.heap.marshaller);
    ...
    fn load_class_object(&self, ptr: &Pointer) -> ExecResult<(Box<StructValue>, ObjectHeader)> {
        let object_val = self.heap.load_object(ptr)?;
    ...
    fn addr_of_ref(&mut self, target: &ir::Ref) -> ExecResult<Pointer> {
        match target {
            ...
            ir::Ref::Result => {
                self.stack.current_frame()?.get_result_ptr().map_err(|err| {
            ...
            ir::Ref::Arg(id) => {
                self.stack.current_frame()?.get_arg_ptr(*id).map_err(|err| {
                    let msg = err.to_string();
                    ExecError::illegal_state(msg)
                })
            },
            ...
            ir::Ref::Local(id) => {
                self.stack.current_frame()?.get_local_ptr(*id).map_err(|err| {
                    let msg = err.to_string();
                    ExecError::illegal_state(msg)
                })
            },
    ...
    fn exec_instruction(
        ...
        self.stack.current_frame_mut()?.set_debug_source(source.cloned());
    ...
    pub fn dynalloc(&mut self, ty: &ir::Type, len: usize, trace: bool) -> ExecResult<Pointer> {
        ...
        let ptr = self.heap.alloc(ty.clone(), len, trace)?;
        ...
        let pointer = match instance_type {
            ...
            other => {
                return Err(ExecError::illegal_state(format!(
                    ...
                    other.to_pretty_string(self.metadata())
                )));
            }
        };
    ...
    fn static_array_element_pointer(&mut self, array_ptr: &Pointer, index: i32) -> ExecResult<Pointer> {
        let ir::Type::Array { element: element_type, .. } = &array_ptr.ty else {
            return Err(ExecError::illegal_state(&format!(
                ...
                array_ptr.ty.to_pretty_string(self.metadata()),
            )));
        };
    ...
    fn exec_length(
        ...
        let length = match of_type {
            ...
            ir::Type::Object(..) => {
                ...
                let header = self.heap.load_array_header(&array_ptr)?;
                header.len
            }
    ...
    fn exec_virtual_call(
        ...
        let (self_val, func) = if self_type.is_abstract() {
            ...
            let func = self
                .metadata()
                .get_interface_method(&def_type, iface_ref, method)
                .ok_or_else(|| {
                    ExecError::illegal_state(format!(
                        "missing implementation of {} for {}",
                        self.metadata().iface_name(iface_ref),
                        def_type.to_pretty_string(self.metadata())
                    ))
                })?;
    ...
    fn exec_is_type(
        ...
        let object_header = self.heap.load_object_header(&a_ptr)?;
    ...
    pub fn load_lib(&mut self, lib: &ir::Library) -> ExecResult<()> {
        ...
        if !lib.init.instructions.is_empty() {
            ...
            self.stack.push(Rc::new("<init>".to_string()), init_stack_size);
    ...
    fn load_type_info(&mut self, ty: &ir::Type) -> ExecResult<DynValue> {
        if let Some(global_ref) = self.typeinfo_map.find_by_key(&ty)
            && let Some(global_value) = self.globals.get(global_ref)
        {
    ...
    fn new_type_info(&mut self, for_type: &ir::Type, name: &str, flags: u64) -> ExecResult<DynValue> {
        let global_ref = ir::GlobalRef::StaticTypeInfo(Rc::new(for_type.clone()));

        assert!(self.typeinfo_map.find_by_key(for_type).is_none(), "new_type_info: type info must not already exist in typeinfo map");
        assert!(self.globals.get(&global_ref).is_none(), "new_type_info: type info must not already exist in globals map");

        let type_info_type = &ir::TYPEINFO_ID.to_class_ptr_type([]);
        self.heap.marshaller.create_native_type(&type_info_type)?;

        let type_info_struct_index = self.heap.marshaller.get_type_index(&ir::TYPEINFO_ID.to_struct_type([]))?;

        let mut type_info_value = StructValue {
            type_index: type_info_struct_index,
            fields: Vec::new(),
        };

        let name_val = if let Some(name_string) = self.metadata().find_string_id(name) {
            self.load_string_lit(name_string)?
        } else {
            self.create_string(name, true)?
        };

        type_info_value.fields.push(name_val); // field 0: name
        ...
        type_info_value.fields.push(DynValue::U64(flags)); // field 4: flags
        ...
        self.globals.insert(global_ref.clone(), global_val);
