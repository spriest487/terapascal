...
fn invoke_method(state: &mut Vm) -> ExecResult<()> {
    ...
    let method_global_index = method_info_val[ir::METHODINFO_IMPL_FIELD]
        .as_pointer()
...
pub(super) fn tan(state: &mut crate::Vm) -> ExecResult<()> {
    let val = load_single(state, &ir::ArgID(0).to_ref())?;
...
pub fn system_funcs() -> impl IntoIterator<Item=(&'static str, BuiltinFn, ir::Type, Vec<ir::Type>)> {
    let items = [
        ...
        ("FreeMem", free_mem, ir::Type::Nothing, vec![
            ir::Type::U8.ptr()
        ]),
        ("ArrayLengthInternal", array_length, ir::Type::I32, vec![
            ir::Type::any()
        ]),
        ("ArrayCreateInternal", array_create, ir::Type::Nothing, vec![
        ...
        ("RandomSingle", random_single, ir::Type::F32, vec![
            ir::Type::F32, ir::Type::F32
        ]),
        ("Time", time, ir::Type::F64, vec![]),
        ("Pow", pow, ir::Type::F32, vec![
            ir::Type::F32, ir::Type::F32
        ]),
        ("Sqrt", sqrt, ir::Type::F32, vec![
            ir::Type::F32
        ]),
        ("Sin", sin, ir::Type::F32, vec![
            ir::Type::F32
        ]),
        ("ArcSin", arc_sin, ir::Type::F32, vec![
            ir::Type::F32
        ]),
        ("Cos", cos, ir::Type::F32, vec![
            ir::Type::F32
        ]),
        ("ArcCos", arc_cos, ir::Type::F32, vec![
            ir::Type::F32
        ]),
        ("Tan", tan, ir::Type::F32, vec![
            ir::Type::F32
        ]),
        ("ArcTan", arc_tan, ir::Type::F32, vec![
            ir::Type::F32
        ]),
        ("Infinity", infinity, ir::Type::F32, vec![]),
        ("IsInfinite", is_infinite, ir::Type::Bool, vec![
            ir::Type::F32
        ]),
        ("NaN", nan, ir::Type::F32, vec![]),
        ("IsNaN", is_nan, ir::Type::Bool, vec![
            ir::Type::F32
        ])
    ];
